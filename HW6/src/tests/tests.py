...
#  assertTrue() или assertFalse() для проверки условия; assertRaises() для проверки,
#  что метод порождает исключение. Эти методы используются вместо обычного assert для того
...
#  ) позволяют определять инструкции, выполняемые перед и после каждого теста,
#  соответственно.
