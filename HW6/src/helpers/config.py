...
USER_HOME_DIR = os.path.expanduser('~')  # домашняя директория пользователя
...
DEFAULT_CONFIG_PATH = [
    ...
    # os.path.realpath(path) - возвращает канонический путь,
    # убирая все символические ссылки (если они поддерживаются).
...
class Config:
    ...
    def get_verbosity_level(level=None, text=False):
        desc, log_level, somestuff = 0,1,2
        levels = (
        ...
        if level is None:
            return ', '.join(x[desc] for x in levels)
        ...
        for x in levels:
            if isinstance(level, str):
                if x[desc] == level.lower():
                    return x[log_level]
            elif x[somestuff]:
                default = x[desc] if text else x[somestuff]
                break
